...
class ProductDetailView(APIView): # получение конкретного продукта по Id
    ...
    def get(self, request, pk):
        ...
        for item in products:
            item['img'] = "https://itemclub.shop" +  item['img']
        return JsonResponse(products, safe=False)


class ListTypeView(APIView): # получение списка продуктов по типу
    permission_classes = [AllowAny]
    ...
    def get(self, request, type_id):
        ...
        for product in data:
            product['img'] = "https://itemclub.shop" + product['img']
        return JsonResponse(data=data, safe=False)


class ListBrandView(APIView): # получение списка продуктов по типу
    permission_classes = [AllowAny]
